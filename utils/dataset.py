...
class DAGMdataset(data.Dataset):
	def __init__(self, root, transform, target_transform, mode='Test', k_shot=0, save_dir=None, obj_name=None):
		...
		len_ = 0
		for cls in meta_info:
			len_ += len(meta_info[cls])
		print(len_)
...
class VisaDataset(data.Dataset):
	def __init__(self, root, transform, target_transform, mode='test', k_shot=0, save_dir=None, obj_name=None):
		...
		self.aux_target_transform = transforms.Compose([
	...
	def __getitem__(self, index):
		...
		if anomaly == 0:
			...
			img_mask = np.array(Image.open(os.path.join(self.root, mask_path)).convert('L')) > 0
			img_mask = Image.fromarray(img_mask.astype(np.uint8) * 255, mode='L')
		aux_img_mask = img_mask
		img = self.transform(img) if self.transform is not None else img
		img_mask = self.target_transform(
			img_mask) if self.target_transform is not None and img_mask is not None else img_mask
		img_mask = [] if img_mask is None else img_mask

		aux_img_mask = self.aux_target_transform(aux_img_mask) if img_mask is not None else img_mask
		aux_img_mask = [] if img_mask is None else aux_img_mask

		return {'img': img, 'img_mask': img_mask, 'cls_name': cls_name, 
		  		'anomaly': anomaly, 'img_path': os.path.join(self.root, img_path), 'aux_img_mask':aux_img_mask}
...
class MVTecDataset(data.Dataset):
	...
	def __getitem__(self, index):
		...
		return {
			'img': img, 'img_mask': img_mask, 'cls_name': cls_name, 
			'anomaly': anomaly, 'img_path': os.path.join(self.root, img_path), 'aux_img_mask': aux_img_mask
		}

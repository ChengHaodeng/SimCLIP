...
import open_clip
from utils.memory_comparison import memory, get_normal_scoremap
...
def test(args):
    ...
    for idx, items in enumerate(test_dataloader):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            # ----------------------------------------------------------------------------------------------
            # Get test image coarse score maps
            ...
            for layer in range(len(patch_tokens)):
                ...
                H = int(np.sqrt(L))
                anomaly_map_before = F.interpolate(anomaly_map.clone().detach().permute(0, 2, 1).view(B, 2, H, H),size=img_size, mode='bilinear', align_corners=False)
                anomaly_map_before = torch.softmax(anomaly_map_before, dim=1)[:, 1, :, :]
                anomaly_maps_before.append(ndimage.gaussian_filter(anomaly_map_before.cpu().numpy(), sigma=7))

                anomaly_map = torch.softmax(anomaly_map.permute(0, 2, 1).view(B, 2, H, H), dim=1)[:, 1, :, :]
                anomaly_maps.append(anomaly_map.cpu().numpy())
            
            anomaly_map = np.sum(anomaly_maps, axis=0).flatten()
            anomaly_maps_before = np.sum(anomaly_maps_before, axis=0)
            # ----------------------------------------------------------------------------------------------
            ...
            if args.mode == 'few_shot':
                ...
                for idx, p in enumerate(patch_tokens):
                    ...
                    anomaly_map_few_shot = np.min((1 - cos)*0.5, 0).reshape(1, 1, height, height)
                    
                    # get aligned feature of specified layer
                    cos_weights = np.max((1 + cos)*0.5, axis=0) # cos_weights in [0, 1]
                    sim_idx = np.argmax((1 + cos)*0.5, axis=0)
                    align_feat_map = ((mem_features[cls_name[0]][idx])[sim_idx, :]).unsqueeze(dim=0) # [1, 1369, 1024]
                    align_feat_map = torch.cat([torch.zeros(1, 1, 1024).to(device), align_feat_map], dim=1).to(device) # [1, 1370, 1024]
                    aligned_feature_maps.append(align_feat_map)
                    
                    anomaly_map_few_shot = F.interpolate(torch.tensor(anomaly_map_few_shot),
                                                         size=img_size, mode='bilinear', align_corners=True)
                    anomaly_maps_few_shot.append(anomaly_map_few_shot[0].cpu().numpy())
                
                # get max value of the Add(anomaly_map, anomaly_map_few_shot)
                anomaly_map_few_shot = np.sum(anomaly_maps_few_shot, axis=0)

                norm_anomaly_map = get_normal_scoremap(cls_name[0], aligned_feature_maps, linearlayer, text_prompts) # [1, 37, 37]
                norm_anomaly_map = norm_anomaly_map.flatten() # [1369,]
                norm_anomaly_map = norm_anomaly_map * cos_weights
                revise_anomaly_map = np.clip(anomaly_map - norm_anomaly_map, 0, 4)
                revise_anomaly_map = revise_anomaly_map.reshape(1, 1, 37, 37)
                anomaly_map = F.interpolate(torch.tensor(revise_anomaly_map), size=img_size, mode='bilinear', align_corners=True)
                anomaly_map = ndimage.gaussian_filter(anomaly_map.squeeze(0), sigma=7)

            results['anomaly_maps_before'].append(anomaly_maps_before + anomaly_map_few_shot)
            results['anomaly_maps'].append(anomaly_map + anomaly_map_few_shot)
    ...
    for obj in obj_list:
        ...
        for idxes in range(len(results['cls_names'])):
            if results['cls_names'][idxes] == obj:
                ...
                pr_sp_tmp.append(np.max(results['anomaly_maps_before'][idxes]))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mode", type=str, default="few_shot", help="t")
    # few shot
    parser.add_argument("--k_shot", type=int, default=1, help="e.g., 10-shot, 5-shot, 1-shot")
